...
object TweetLength {
  ...
  def tweetRemainingCharsCount(tweetText: Signal[String]): Signal[Int] = new Signal(140 - tweetLength(tweetText()))

  def colorForRemainingCharsCount(remainingCharsCount: Signal[Int]): Signal[String] =
    new Signal(
      if (remainingCharsCount() > 14) "green"
      else if (remainingCharsCount() < 15 && remainingCharsCount() > -1) "orange"
      else "red"
    )
